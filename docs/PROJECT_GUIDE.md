...
KOSPI·KOSDAQ 상장 종목에 대해 **일별 뉴스**와 **주가 급등(기본 일 20% 이상)** 패턴을 연결하고, **익일(다음 거래일) 급등 후보**를 HTML 리포트로 출력하는 도구입니다.

예측은 **휴리스틱(과거 급등–뉴스 키워드 교집합·종목명·테마·키워드 피드백)** 과 **ML 랭커(HistGradientBoosting, KOSPI·시세 피처 포함)** 를 함께 쓰며, `PRED_USE_ML_RANKER=0` 이면 휴리스틱만 사용합니다. 표시 예측 상승률은 상위 후보 **순위 기준 20~35%** 구간으로 매핑합니다.
...
상장 + OHLCV(캐시) → 일간 수익률
        ↓
캘린더 일자별 뉴스(JSON) → 훈련 구간 BreakoutEvent (급등일 키워드)
        ↓
관측일 T: early 뉴스 → ML/휴리스틱 점수 → 상위 40후보 + 표시 예측%
        ↓
...
| 구간 | 설정 | 용도 |
|------|------|------|
| 훈련 | `TRAIN_START_DEFAULT` ~ `TEST_START` 직전 | `BreakoutEvent`, ML 학습 라벨 |
| 테스트/리포트 | `TEST_START` 이후 거래일 | HTML·정확도 추적 |
