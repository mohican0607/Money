"""
전체 뉴스 맥락(TF-IDF) ↔ 과거 급등일 뉴스 프로필 유사도 — 키워드 교집합만으로는 놓치던 급등 후보 보완.

파이프라인 역할:
  - ``make_news_ctx_bundle``: 관측일 T 직전 구간 뉴스로 어휘·IDF·급등 term lift·종목별 프로필 생성
  - ``affinity_candidate_codes``: 당일 early 뉴스 벡터와 종목 프로필 코사인 유사도로 ML 후보 풀 확장
  - ``context_feature_vector`` / ``feats_for_code``: ML 랭커 피처 5개(news_cos_code 등) 및 휴리스틱 점수

학습·추론 모두 동일 bundle 을 쓰며, joblib 캐시에 ``news_ctx`` 가 함께 저장됩니다.
"""
