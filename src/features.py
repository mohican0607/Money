...
# 조사·수량·상투 잔재 (generic 밖에 남는 잡토큰)
_FILLER_TOKEN_STOP = frozenset(
    {
        "원을",
        "원으로",
        "원이",
        ...
        "정도",
        "수준으로",
        "플랫폼",  # 단독으론 너무 흔함 — 테마 프로필·업종 겹침으로만 살림
    }
)

_CURRENCY_TOKEN_RE = re.compile(r"^[\d,\.]+원?$")
_JOSA_TAIL_RE = re.compile(
    r"(을|를|이|가|은|는|의|에|로|과|와|도|만|부터|까지|에서|으로)$"
)
...
def is_substantive_keyword(k: str) -> bool:
    """
    급등·테마 신호로 쓸 만한 실질 키워드인지.

    기사 상투어·언론사명·조사·금액 토큰은 제외합니다.
    """
    t = clean_keyword_token(k)
    if not is_valid_keyword(t):
        return False
    low = t.lower()
    if is_generic_market_keyword(low):
        return False
    if low in _MEDIA_META_STOP or low in _FILLER_TOKEN_STOP:
        return False
    if _CURRENCY_TOKEN_RE.match(low):
        return False
    # 2~3글자인데 조사 꼬리만 남은 형태
    if len(low) <= 3 and _JOSA_TAIL_RE.search(low):
        return False
    return True


def filter_specific_keywords(keywords: list[str] | frozenset[str] | set[str]) -> frozenset[str]:
    """범용·상투·언론 메타를 제외한 교집합·프로필용 실질 키워드 집합."""
    out: set[str] = set()
    for k in keywords:
        t = clean_keyword_token(k)
        if not is_substantive_keyword(t):
            continue
        out.add(t)
